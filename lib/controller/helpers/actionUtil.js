...
module.exports = {
	...
	populateEach: function(query, req){
		...
		return _(_options.associations).reduce(function populateEachAssociation(query, association){
			...
			if(shouldPopulate) {
				...
				if(association.type == 'model'){ //as opposed to collection
					populationLimit = 1;
				}

				if(_.isNumber(populationLimit)){
					crit.limit = populationLimit;
				}

				return query.populate(association.alias, crit);
			} else {
				return query;
			}
		}, query);
	...
	/**
	   ...
	   *
	   * @param  {Request} req
	   * @param  {Array} [black] Array of parameters to "blacklist".
	   * @return {Object}            the WHERE criteria object
	   */
	parseCriteria: function(req, black){
		...
		var blacklist = black || req.options.criteria && req.options.criteria.blacklist;
	...
	/**
	   ...
	   * @return {Object}
	   */
	parseValues: function(req, black){
		...
		// Validate blacklist to provide a more helpful error msg.
		var blacklist = black || req.options.values.blacklist;
	...
	/**
	   ...
	   * @param  {Request} req
	   * @param  {String} modelName (optional) the model name.
	   * @return {WLCollection}
	   */
	parseModel: function(req, modelName){

		// Ensure a model can be deduced from the request options.
		var model = modelName || req.options.model || req.options.controller;
