...
module.exports = function updateOneRecord(req, res){
	...
	var opts = _.defaults({}, this.getQueryConfiguration(req, 'update'));

	// Create `values` object (monolithic combination of all parameters)
	// But omit the blacklisted params (like JSONP callback param, etc.)
	var values = actionUtil.parseValues(req, opts.blacklist);
	...
	Model.findOne(pk).populateAll().exec(function found(err, model){

		if(err) {return res.serverError(err);}

		if(!model) {return res.notFound();}

		self.canUpdate(req, model, function(err, value){
		if(err) {return res.serverError(err);}

		if(!value){return res.forbidden({
			error: 'You are not authorized to update this ' + Model.globalId.toLowerCase()
		});}

		self.beforeUpdate(req, values, model, function(err, result){
			if(err){return res.serverError(err);}
			...
			Model.update(pk, values).exec(function updated(err, records){
				...
				Q.exec(function foundAgain(err, populatedRecord){
					...
					self.export(req, populatedRecord, function(err, modelJSON){
					if(err){return res.serverError(err);}

					self.afterUpdate(req, populatedRecord, function(err){
						if(err) {return res.serverError(err);}
